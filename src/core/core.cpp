...
bool BattleState::submit(Command c,std::string* err) {
  if(c.requested_ms<=now_ms_ || c.requested_ms%kTickMs) { if(err)*err="command time must be a future multiple of 10 ms"; emit(EventType::Rejected,0,0,0,"unrepresentable command time"); return false; }
...
Milliseconds BattleState::advance_ticks(std::uint32_t n){const auto start=now_ms_;for(std::uint32_t i=0;i<n&&!finished_;++i)tick();return now_ms_-start;} Milliseconds BattleState::advance_to(Milliseconds t){if(t<now_ms_||t%kTickMs)throw std::invalid_argument("advance target must be a future multiple of 10 ms");return advance_ticks(static_cast<std::uint32_t>((t-now_ms_)/kTickMs));}
std::vector<EntityView> BattleState::observe()const{std::vector<EntityView>o;for(auto&e:entities_)if(alive(e)&&!e.concealed){const auto* stats=data_.find(e.kind,e.level,e.supercharged?"supercharged":"normal");o.push_back({e.id,e.kind,e.side,e.level,e.pos,e.hp,e.max_hp,e.range,e.target,e.next_action,stats?stats->category:EntityCategory::Other,stats?stats->target_focus:TargetFocus::Any,stats?stats->flying:false,stats?stats->heals:false,stats?stats->footprint_width:1,stats?stats->footprint_height:1,e.supercharged,e.underground,e.invisible_to_defenses_until>now_ms_,e.deployment_rage_until>now_ms_});}return o;}
