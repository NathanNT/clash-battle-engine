...
std::uint64_t BattleState::state_hash() const {
  // Snapshot V18 remains the current complete, versioned state representation.
  // Hashing it
  // keeps the determinism oracle aligned with every persisted runtime field.
  const auto& canonical = snapshot().canonical;
...
std::uint64_t BattleState::scenario_fingerprint() const {
...
bool BattleState::restore(const Snapshot& snap,std::string* error) {
  ...
  for(const char c:version_text) { if(version>18){if(error)*error="unsupported snapshot format";return false;} version=version*10+(c-'0'); }
  if(version<2||version>18){if(error)*error="unsupported snapshot format";return false;}
  const bool v3=version>=3,v4=version>=4,v5=version>=5,v6=version>=6,v7=version>=7,v8=version>=8,v9=version>=9,v10=version>=10,v11=version>=11,v12=version>=12,v13=version>=13,v14=version>=14,v15=version>=15,v16=version>=16,v17=version>=17,v18=version>=18; BattleState c(data_,scenario_); std::size_t n; int b;
  ...
  if(!(s>>n)){if(error)*error="truncated army";return false;} c.scenario_.army.clear(); for(std::size_t i=0;i<n;++i){int k;ArmySlot a;if(!(s>>k>>a.level>>a.count)||k<0||k>=int(Kind::Count)){if(error)*error="invalid army";return false;}a.kind=Kind(k);c.scenario_.army.push_back(a);}
  ...
  if(!(s>>n)){if(error)*error="truncated entities";return false;} c.entities_.clear(); for(std::size_t i=0;i<n;++i){Entity e;int k,side,ranged,splash,flying,heals,focus,type,defenses;EntityId target;int has_waypoint;if(!(s>>e.id>>k>>side>>e.level>>e.pos.x>>e.pos.y>>e.hp>>e.max_hp>>e.damage>>e.healing>>e.range>>e.min_range>>e.speed>>e.radius>>e.splash_radius>>e.cooldown>>e.next_action>>e.next_path>>e.time_between_bursts>>e.activation_housing>>e.shots_per_burst>>e.shots_in_burst>>ranged>>splash>>flying>>heals>>focus>>type>>defenses>>target>>has_waypoint)){if(error)*error="truncated entity";return false;}double wx,wy;int concealed=0,underground=0;Milliseconds invisible_to_defenses_until=0,inferno_lock_elapsed=0,next_aura_action=0,deployment_rage_until=0;EntityId inferno_lock_target=0;if(!(s>>wx>>wy)||(v3&&!(s>>e.trigger_range>>concealed))||(v4&&!(s>>e.earthquake_hits))||(v5&&!(s>>e.multi_target_count))||(v6&&!(s>>e.supercharged))||(v8&&!(s>>underground))||(v9&&!(s>>invisible_to_defenses_until))||(v10&&!(s>>inferno_lock_elapsed>>inferno_lock_target))||(v14&&!(s>>next_aura_action))||(v15&&!(s>>deployment_rage_until))||(v17&&!(s>>e.opening_long_shots_remaining))||k<0||k>=int(Kind::Count)||side<0||side>1||focus<0||focus>int(TargetFocus::DefensesOnly)||type<0||type>int(TargetType::Both)){if(error)*error="invalid entity";return false;}e.kind=Kind(k);e.side=Side(side);e.ranged=ranged;e.splash=splash;e.flying=flying;e.heals=heals;e.target_focus=TargetFocus(focus);e.target_type=TargetType(type);e.defenses=defenses;if(const auto* st=c.data_.find(e.kind,e.level,e.supercharged?"supercharged":"normal")){e.death_damage=st->death_damage;e.death_splash_radius=st->death_splash_radius;e.death_damage_delay=st->death_damage_delay;e.speed=st->movement_tiles_per_second;if(!v3)e.trigger_range=st->trigger_range;if(!v14&&st->aura_cooldown>0) next_aura_action=c.now_ms_+st->aura_cooldown;if(!v17)e.opening_long_shots_remaining=st->opening_long_shot_count;}e.concealed=concealed;e.underground=underground;e.invisible_to_defenses_until=invisible_to_defenses_until;e.inferno_lock_elapsed=inferno_lock_elapsed;e.inferno_lock_target=inferno_lock_target;e.next_aura_action=next_aura_action;e.deployment_rage_until=deployment_rage_until;if(target)e.target=target;if(has_waypoint)e.waypoint=Vec2{wx,wy};c.entities_.push_back(e);}
