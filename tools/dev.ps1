...
& cmake -S $ProjectRoot -B $BuildDir -G "Visual Studio 17 2022" -A x64 -DCLASH_BATTLE_ENGINE_BUILD_VIEWER=ON -DCLASH_BATTLE_ENGINE_BUILD_TESTS=ON -DCLASH_BATTLE_ENGINE_BUILD_PYTHON=OFF
...
if ($Action -eq "test") {
    ...
    $Executable = Join-Path $BuildDir "$Configuration/clash_battle_engine_viewer.exe"
    & $Executable @RunArguments
    if ($LASTEXITCODE -ne 0) { throw "Viewer failed." }
} elseif ($Action -eq "headless") {
    $Executable = Join-Path $BuildDir "$Configuration/clash_battle_engine.exe"
